...
link<-
  ...
  str_subset("fft-mh|mental") %>% # find those that end in xlsx
  .[[1]]     



#Download File
destfile <- "Mental.xlsm"
curl::curl_download(link, destfile)

FFTMH <- read_excel(destfile, sheet = "Trusts", skip = 2) |> 
  drop_na(3) |> 
...
Loaded_Date <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_FriendsAndFamilyTest_Mental_Health_Trusts1 ")) %>% 
  collect() %>% 
...
Loaded_No <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_FriendsAndFamilyTest_Mental_Health_Trusts1 ")) %>% 
  collect() %>% 
  select(Effective_Snapshot_Date) |> 
  group_by(Effective_Snapshot_Date) |> 
  summarise(Count = n()) |> 
...
Load <- function() {
  ...
  if(Loaded_Date == thismonth) {
    print(paste("Not Loading FFT Mental Health Data:",Loaded_No,"records for" , Loaded_Date, "already loaded"))
    beep(9)
    } else {
    dbWriteTable(SQL_CONNECTION, Id(schema = "dbo", table = "zzz_FriendsAndFamilyTest_Mental_Health_Trusts1"), FFTMH, append = TRUE)
    print(paste("Loaded FFT Mental Health Data for ", thismonth))
    }
}
