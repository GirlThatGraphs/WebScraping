...
link<-
  ...
  str_subset("4WW") |>  # find those that end in xlsx
  head(1)

#Download File
destfile <- "MHWaitList.csv"
curl::curl_download(link, destfile)

MHWL <- read.csv("MHWaitList.csv") |> 
...
Loaded_Date <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_Mental_Health_Waiting_Lists")) %>% 
  collect() %>% 
...
Loaded_No <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_Mental_Health_Waiting_Lists")) %>% 
  collect() %>% 
  select(Effective_Snapshot_Date) |> 
  group_by(Effective_Snapshot_Date) |> 
  summarise(Count = n()) |> 
...
Load <- function() {
  ...
  if(Loaded_Date == effectdate) {
    print(paste("Not Loading Mental Health Wait List Data:",Loaded_No,"records for" , Loaded_Date, "already loaded"))
    beep(9)
  } else {
    dbWriteTable(SQL_CONNECTION, Id(schema = "dbo", table = "zzz_Mental_Health_Waiting_Lists"), MHWL, append = TRUE)
    print(paste("Loaded Mental Health Wait List Data for ", effectdate))
  }
}
