...
link<-
  ...
  str_subset("ae|accident") %>% # find those that end in xlsx
  .[[1]]     
...
FFTAE <- read_excel(destfile, sheet = "Trusts", skip = 2) |> 
  drop_na() |> 
  set_colnames(c("Region_Code","Trust_Code","Trust_Name","Total Responses","Total Eligible","Percentage Positive","Percentage Negative",
                 "Very Good","Good","Neither Good nor Poor","Poor","Very Poor","Don't Know",
                 "Mode SMS","Mode Electronic Discharge","Mode Paper Discharge","Mode Paper Home","Mode Telephone","Mode Online","Mode Other")) |> 
  select(-Trust_Name) |> 
  pivot_longer(3:19,values_transform = list(value = as.numeric)) |>
...
Loaded_Date <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_FriendsAndFamilyTest_A_And_E_Trust_Data1")) %>% 
  collect() %>% 
...
Loaded_No <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_FriendsAndFamilyTest_A_And_E_Trust_Data1")) %>% 
  collect() %>% 
  select(Effective_Snapshot_Date) |> 
  group_by(Effective_Snapshot_Date) |> 
  summarise(Count = n()) |> 
...
Load <- function() {
  ...
  if(Loaded_Date == thismonth) {
    print(paste("Not Loading FFT A&E Data:",Loaded_No,"records for" , Loaded_Date, "already loaded"))
    beep(9)
  } else {
    dbWriteTable(SQL_CONNECTION, Id(schema = "dbo", table = "zzz_FriendsAndFamilyTest_A_And_E_Trust_Data1"), FFTAE, append = TRUE)
    print(paste("Loaded FFT A&E Data for ", thismonth))
  }
}
