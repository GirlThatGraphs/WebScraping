...
link<-
  ...
  str_subset("fft-mat|FFT-mat|maternity") %>% # find those that end in xlsx
  .[[1]]     



#Download File
destfile <- "Maternity.xlsm"
curl::curl_download(link, destfile)

FFTMat1 <- read_excel(destfile, sheet = "Trusts Q1", skip = 8) |> 
  drop_na(3) |> 
  set_colnames(c("Area_Team_Code","Trust_Code","Trust_Name","Total Responses","Percentage Positive","Percentage Negative",
  ...
  pivot_longer(2:18,values_transform = list(value = as.numeric)) |>
  ...
  mutate(Measure_Value_Str = as.character(Measure_Value),
         ...
         AuditKey = "") |> 
  mutate(Question = "Q1") |> 
  select(1,11,4,2,3,6,5:10)



FFTMat2 <- read_excel(destfile, sheet = "Trusts Q2", skip = 8) |> 
  drop_na(3) |> 
  set_colnames(c("Area_Team_Code","Trust_Code","Trust_Name","Total Responses","Total Eligable","Percentage Positive","Percentage Negative",
                 "Very Good","Good","Neither Good nor Poor","Poor","Very Poor","Don't Know",
                 "Mode SMS","Mode Electronic Discharge","Mode Electronic Home","Mode Paper Discharge","Mode Paper Home","Mode Telephone","Mode Online","Mode Other")) |> 
  mutate(Trust_Code = case_when(Trust_Name == "England (including Independent Sector providers)" ~ "ENG - including Independent Sector Providers",
                                Trust_Name == "England (excluding Independent Sector providers)" ~ "ENG - excluding Independent Sector Providers",
...
FFTMat3 <- read_excel(destfile, sheet = "Trusts Q3", skip = 8) |> 
  drop_na(3) |> 
  set_colnames(c("Area_Team_Code","Trust_Code","Trust_Name","Total Responses","Percentage Positive","Percentage Negative",
                 "Very Good","Good","Neither Good nor Poor","Poor","Very Poor","Don't Know",
                 "Mode SMS","Mode Electronic Discharge","Mode Electronic Home","Mode Paper Discharge","Mode Paper Home","Mode Telephone","Mode Online","Mode Other")) |> 
...
FFTMat4 <- read_excel(destfile, sheet = "Trusts Q4", skip = 8) |> 
  drop_na(3) |> 
  set_colnames(c("Area_Team_Code","Trust_Code","Trust_Name","Total Responses","Percentage Positive","Percentage Negative",
                 "Very Good","Good","Neither Good nor Poor","Poor","Very Poor","Don't Know",
                 "Mode SMS","Mode Electronic Discharge","Mode Electronic Home","Mode Paper Discharge","Mode Paper Home","Mode Telephone","Mode Online","Mode Other")) |> 
...
FFTMat <- rbind(FFTMat1, FFTMat2, FFTMat3, FFTMat4)
...
Loaded_Date <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_FriendsAndFamilyTest_Maternity_Trusts1")) %>% 
  collect() %>% 
...
Loaded_No <- tbl(SQL_CONNECTION, in_schema("dbo","zzz_FriendsAndFamilyTest_Maternity_Trusts1")) %>% 
  collect() %>% 
  select(Effective_Snapshot_Date) |> 
  group_by(Effective_Snapshot_Date) |> 
  summarise(Count = n()) |> 
...
Load <- function() {
  ...
  if(Loaded_Date == thismonth) {
    print(paste("Not Loading FFT Maternity Data:",Loaded_No,"records for" , Loaded_Date, "already loaded"))
    beep(9)
    } else {
    dbWriteTable(SQL_CONNECTION, Id(schema = "dbo", table = "zzz_FriendsAndFamilyTest_Maternity_Trusts1"), FFTMat, append = TRUE)
    print(paste("Loaded FFT Maternity Data for ", thismonth))
    }
}
